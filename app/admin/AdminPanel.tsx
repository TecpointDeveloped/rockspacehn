...
export function AdminPanel({ initial, email }: { initial: CmsContent; email: string }) {
  ...
  function addFilm() {
    setContent((current) => ({ ...current, films: [{ slug: `nueva-lamina-${current.films.length + 1}`, name: "Nueva lámina", short: "Beneficio principal", description: "Descripción", benefits: [], category: "Lámina frontal", categoryKey: "flexible", availability: "Disponible", stock: 0, variants: [], compatibility: [], specs: [], tone: "clear", image: "/images/products/film-hd.webp", alt: "Lámina Rock Space" }, ...current.films] }));
    setNotice("Nueva lámina preparada. Cargue la imagen, complete la información y publique los cambios.");
  }
  ...
  return <div className={styles.page}>
    ...
    <div className={styles.workspace}>
      ...
      <section className={styles.editor}>
        {tab === "films" && <div className={styles.uploadCallout}><div><small>NUEVA ÁREA DE CARGA</small><strong>¿Falta una lámina en el catálogo?</strong><span>Creá el registro, subí la fotografía en alta calidad y completá existencias, variantes y especificaciones.</span></div><button type="button" onClick={addFilm}>Subir una nueva lámina +</button></div>}
        ...
        {tab === "support" && <div className={styles.formGrid}><h2>Contactos de Rock Space</h2><p className={styles.intro}>Ambos números se muestran con el prefijo internacional +504 de Honduras.</p><Field label="Atención al cliente / ventas (+504)" value={content.support.salesWhatsappNumber} onChange={(value) => setContent({ ...content, support: { ...content.support, salesWhatsappNumber: value.replace(/\D/g, "") } })} /><Field label="Ventas y soporte (+504)" value={content.support.supportWhatsappNumber} onChange={(value) => setContent({ ...content, support: { ...content.support, supportWhatsappNumber: value.replace(/\D/g, "") } })} /></div>}
      </section>
    </div>
