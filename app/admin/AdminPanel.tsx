...
export function AdminPanel({ initial, email }: { initial: CmsContent; email: string }) {
  ...
  function addFilm() {
    setContent((current) => ({ ...current, films: [...current.films, { name: "Nueva lámina", short: "Beneficio principal", description: "Descripción", benefits: [], category: "Lámina frontal", availability: "Confirmar disponibilidad", specs: [], tone: "clear", image: "/images/products/film-hd.webp", alt: "Lámina Rock Space" }] }));
  }
  ...
  return <div className={styles.page}>
    ...
    <div className={styles.workspace}>
      ...
      <section className={styles.editor}>
        ...
        {tab === "support" && <div className={styles.formGrid}><h2>Soporte del equipo Rock Space</h2><p className={styles.intro}>La comunicación pública pertenece a la empresa, sin nombres personales.</p><Field label="WhatsApp (504...)" value={content.support.whatsappNumber} onChange={(value) => setContent({ ...content, support: { ...content.support, whatsappNumber: value.replace(/\D/g, "") } })} /></div>}
      </section>
    </div>
