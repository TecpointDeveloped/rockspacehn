...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const { slug } = await params; const cms = await getCmsContent(); const machine = cms.machines.find((item) => item.slug === slug && item.active !== false) || getMachine(slug); if (!machine) return {};
...
export default async function MachinePage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params; const cms = await getCmsContent(); const machine = cms.machines.find((item) => item.slug === slug && item.active !== false); if (!machine) notFound();
  const learningItems = [
    ...
    <>
      <section className="product-hero-v2">
        <div className="shell product-hero-v2-grid">
          ...
          <div className="product-stage"><div className="stage-ring"/><Image src={machine.image} alt={`${machine.name} de Rock Space`} width={1200} height={900} priority quality={90} sizes="(max-width: 900px) 94vw, 620px" /><span className="stage-label">ROCK SPACE · HONDURAS</span></div>
        </div>
      </section>
      ...
      <section className="tutorial-dark" id="tutorial"><div className="shell tutorial-detail-grid"><div><span className="eyebrow light">TUTORIAL INTEGRADO</span><h2>Cómo funciona la {machine.name}</h2><p>Reproduzca el tutorial aquí mismo. No hace falta abrir la página del fabricante ni buscar el modelo en otra web.</p><div className="tutorial-chip">▶ VIDEO DENTRO DE ROCKSPACEHN</div></div><VideoEmbed youtubeId={machine.youtubeId} title={machine.videoLabel} spanishSummary={machine.spanishSummary} /></div><div className="shell tutorial-steps">{machine.steps.map((step) => <article key={step.title}><h3>{step.title}</h3><p>{step.text}</p></article>)}</div><div className="shell learning-path-wrap"><span className="eyebrow light">RUTA DE APRENDIZAJE</span><LearningPath items={learningItems} /></div></section>
