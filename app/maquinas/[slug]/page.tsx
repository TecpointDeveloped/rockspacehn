...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const { slug } = await params; const cms = await getCmsContent(); const machine = cms.machines.find((item) => item.slug === slug) || getMachine(slug); if (!machine) return {};
...
export default async function MachinePage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params; const cms = await getCmsContent(); const machine = cms.machines.find((item) => item.slug === slug) || getMachine(slug); if (!machine) notFound();
  const learningItems = [
  ...
  return (
    <>
      <section className="product-hero-v2">
        <div className="shell product-hero-v2-grid">
          <div className="product-hero-copy">
            ...
            <div className="button-row"><a className="button button-white" href={whatsappUrlFor(cms.support.supportWhatsappNumber, `Hola, quiero información y disponibilidad de la ${machine.name}.`)} target="_blank" rel="noreferrer">Consultar disponibilidad</a><a className="button button-ghost-light" href="#tutorial">Ver cómo funciona</a></div>
          </div>
          <div className="product-stage"><div className="stage-ring"/><Image src={machine.image} alt={`${machine.name} de Rock Space`} width={1200} height={900} priority quality={88} sizes="(max-width: 900px) 94vw, 620px" /><span className="stage-label">ROCK SPACE · HONDURAS</span></div>
        </div>
      </section>
      ...
      <section className="section shell">
        ...
        <div className="product-gallery">{machine.gallery.map((image, index) => <figure className={index === 0 ? "gallery-wide" : ""} key={image.src}><Image src={image.src} alt={image.alt} width={1300} height={1000} quality={88} sizes={index === 0 ? "(max-width: 900px) 92vw, 660px" : "(max-width: 900px) 92vw, 480px"}/><figcaption>{image.alt}</figcaption></figure>)}</div>
      </section>
      ...
      <section className="tutorial-dark" id="tutorial"><div className="shell tutorial-detail-grid"><div><span className="eyebrow light">TUTORIAL INTEGRADO</span><h2>Cómo funciona la {machine.name}</h2><p>Reproduzca el tutorial aquí mismo. No hace falta abrir la página del fabricante ni buscar el modelo en otra web.</p><div className="tutorial-chip">▶ VIDEO DENTRO DE ROCKSPACEHN</div></div><VideoEmbed youtubeId={machine.youtubeId} title={machine.videoLabel} spanishSummary={(machine as { spanishSummary?: string }).spanishSummary || machine.steps.map((step) => `${step.title}: ${step.text}`).join(" ")} /></div><div className="shell tutorial-steps">{machine.steps.map((step) => <article key={step.title}><h3>{step.title}</h3><p>{step.text}</p></article>)}</div><div className="shell learning-path-wrap"><span className="eyebrow light">RUTA DE APRENDIZAJE</span><LearningPath items={learningItems} /></div></section>

      <section className="section shell two-column-info"><div><SectionHeading eyebrow="ESPECIFICACIONES" title="Todo lo importante, aquí." /><dl className="spec-list">{machine.specs.map((spec) => <div key={spec.label}><dt>{spec.label}</dt><dd>{spec.value}</dd></div>)}</dl><p className="internal-source-note">Información técnica verificada con fichas oficiales de Rock Space y presentada dentro de ROCKSPACEHN.</p></div><aside className="ideal-card"><span className="eyebrow">IDEAL PARA</span><h2>{machine.idealFor}</h2><h3>Láminas / usos compatibles</h3><ul>{machine.compatibleFilms.map((film) => <li key={film}>✓ {film}</li>)}</ul><a className="button button-primary full-button" href={whatsappUrlFor(cms.support.supportWhatsappNumber, `Hola, quiero saber qué láminas recomiendan para la ${machine.name}.`)} target="_blank" rel="noreferrer">Consultar láminas</a></aside></section>
      ...
      <div className="shell back-link"><Link className="text-link" href="/maquinas">← Volver a todas las máquinas</Link></div><SupportCTA machineName={machine.name} />
    </>
  );
}
