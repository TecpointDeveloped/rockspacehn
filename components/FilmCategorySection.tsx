  ...
  return (
    <section className={`catalog-family ${dark ? "catalog-family-dark" : ""}`} id={id}>
      <div className="shell section">
        <div className="catalog-family-head"><div><span className={`eyebrow ${dark ? "light" : ""}`}>CATÁLOGO ROCK SPACE</span><h2>{title}</h2><p>{description}</p></div><span>{String(products.length).padStart(2, "0")} productos</span></div>
        <div className="catalog-product-grid">
          {products.map((product, index) => {
            const variants = availableVariants(product.variants || []);
            const productImages = product.image ? (product.images?.includes(product.image) ? product.images : [product.image]) : [];
            return <article className="catalog-product" key={product.slug || product.name}>
              <div className={`catalog-product-image ${productImages.length > 1 ? "catalog-product-image-pair" : ""}`}>
                ...
                {productImages.length === 0 && <div className="catalog-image-placeholder" aria-label={`Imagen pendiente para ${product.name}`}><span>IMAGEN PENDIENTE</span><strong>{product.sku}</strong><small>Subir desde el administrador</small></div>}
              </div>
              <div className="catalog-product-copy"><span>{product.subcategory || product.category}</span><h3>{product.name}</h3>{product.sku && <b className="catalog-product-sku">SKU {product.sku}</b>}<p>{product.short}</p><ul>{product.benefits.slice(0, 3).map((benefit) => <li key={benefit}>✓ {benefit}</li>)}</ul>
                ...
                <a href={whatsappUrlFor(salesNumber, `Hola, quiero información sobre ${product.name}, SKU ${product.sku || "por confirmar"}.`)} target="_blank" rel="noreferrer">Consultar <b>→</b></a>
              </div>
            </article>;
          })}
        </div>
